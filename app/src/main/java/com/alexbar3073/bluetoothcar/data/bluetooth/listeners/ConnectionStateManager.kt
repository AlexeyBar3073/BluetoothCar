 ...
 *
 * НАЗНАЧЕНИЕ ФАЙЛА:
 * МЕНЕДЖЕР СОСТОЯНИЯ BLUETOOTH СОЕДИНЕНИЯ. Отвечает за установление физического
 * соединения с устройством и мониторинг его состояния согласно ТЗ.
 *
 * ОТВЕТСТВЕННОСТЬ (СОГЛАСНО ТЗ):
 * 1. Выполнение 5 попыток подключения к целевому устройству
 * 2. Мониторинг состояния установленного соединения
 * 3. Уведомление о начале подключения (статус CONNECTING)
 * 4. Уведомление об успешном подключении (статус CONNECTED)
 * 5. Уведомление о разрыве/неудаче подключения (статус DISCONNECTED)
 * 6. Уведомление об ошибках (статус ERROR)
 *

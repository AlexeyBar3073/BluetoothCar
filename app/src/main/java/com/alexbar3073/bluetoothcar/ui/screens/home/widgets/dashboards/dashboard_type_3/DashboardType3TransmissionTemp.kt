...
@Composable
internal fun DashboardType3TransmissionTemp(
    ...
    val animatedTemp by animateFloatAsState(
        targetValue = carData.transmissionTemp.coerceIn(minTemp, maxTemp),
        animationSpec = spring(dampingRatio = 0.8f, stiffness = 80f),
        label = "temp_anim"
    )

    val iconPainter = painterResource(R.drawable.oil_temperature_new)
...
private fun DrawScope.drawTempValueText(
    geometry: DashboardType3Geometry,
    temp: Int,
    hubRadius: Float
) {
    ...
    drawContext.canvas.nativeCanvas.apply {
        ...
        // Смещаем текст вверх относительно центра
...
@Composable
private fun DashboardType3TransmissionTempPreview() {
    BoxWithConstraints(modifier = Modifier.size(250.dp)) {
        ...
        DashboardType3TransmissionTemp(
            carData = CarData(transmissionTemp = 87f),
            geometry = geometry,
